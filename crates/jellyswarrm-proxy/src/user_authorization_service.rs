...
"#,
        ...
        let sessions = if let Some(device) = device {
            ...
            let mut matched: Vec<(AuthorizationSession, Server)> = sessions
                .into_iter()
                .filter(|(session, _)| device.matches(&session.device))
                .collect();

            // `Device::matches` deliberately accepts a device id that has drifted — for non-web
            // clients it compares only a 16-character prefix — so one request can match several
            // stored sessions at once: the client's current one and every one it superseded.
            // Sessions are keyed on `(user, mapping, device_id)`, so re-logging in with a new
            // device id adds a row rather than replacing one, and both then match.
            //
            // Those sessions are not interchangeable. Jellyfin binds a device to an access token
            // when the token is minted and attributes every later request to that device, ignoring
            // the `DeviceId` header. So each stored session pins the upstream to the device id it
            // was created with, and picking a superseded one leaves the client unable to find
            // itself in `GET /Sessions?deviceId=…`. That is where Media Bar's play button gives up
            // on Jellyfin Media Player: it looks its own session up by device id, gets nothing
            // back, and never sends the play command.
            //
            // The sort is stable, so server priority still decides between equally exact matches.
            let requested = normalize_device(&device.device_id);
            matched.sort_by_key(|(session, _)| {
                normalize_device(&session.device.device_id) != requested
            });
            matched
        } else {
            sessions
...
mod tests {
    ...
    /// Jellyfin Media Player's device id is `base64("JellyfinMediaPlayer <version> (<os>)|<epoch>")`,
    /// so two ids generated by the same install a year apart share far more than the 16 characters
    /// `Device::matches` compares. Both therefore match one request, which is what makes the
    /// ordering below load-bearing rather than cosmetic.
    #[test]
    fn a_regenerated_device_id_still_matches_the_session_it_superseded() {
        let old = Device {
        ...
        assert!(
            current.matches(&old),
            "the prefix fallback is what lets a stale session keep matching"
        );
    }

    /// A client that regenerated its device id matches both its current session and the one it
    /// superseded. Each session's upstream token is bound to the device id it was minted with, so
    /// serving the superseded one leaves the client unable to find itself in
    /// `GET /Sessions?deviceId=…` — which is exactly where Media Bar's play button gives up on JMP.
    #[tokio::test]
    async fn an_exactly_matching_device_wins_over_one_that_merely_shares_a_prefix() {
        let (pool, service) = setup_service().await;
        insert_test_server(&pool, "Test Server", "http://localhost:8096").await;
        ...
        let mut auth = Authorization {
            client: "Jellyfin Media Player".to_string(),
            device: "Sosia-Main".to_string(),
            device_id: superseded_device_id.to_string(),
            version: "1.12.0".to_string(),
            token: None,
        };

        // Stored first, so it is the one a naive `.first()` would reach for.
        service
            .store_authorization_session(
                &user.id,
                "http://localhost:8096",
                &auth,
                "superseded-token".to_string(),
        ...
        auth.device_id = current_device_id.to_string();
        service
            .store_authorization_session(
                &user.id,
                "http://localhost:8096",
        ...
        let sessions = service
            .get_user_sessions(&user.id, Some(requesting_device))
            .await
            .unwrap();
        ...
        assert_eq!(
            sessions.len(),
            2,
            "both sessions still match; the fix is which one comes first"
        );
        assert_eq!(sessions[0].0.device.device_id, current_device_id);
        assert_eq!(
            sessions[0].0.jellyfin_token, "current-token",
            "the token decides which device the upstream reports, so it must be the current one"
        );
    }
